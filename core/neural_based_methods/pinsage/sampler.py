...
class ItemToItemBatchSampler(IterableDataset):
    ...
    def __iter__(self):
        # generate a list of samples (q, i) and respective negative items for each q
        # q = query item, i = postive item
        while True:
            # random walk start nodes, randomly selected  
            # namely query nodes q
            heads = torch.randint(
                0, self.g.num_nodes(self.item_type), (self.batch_size,)
            )  
            # length-2 random walk end nodes started from heads
            # namely positive items i
            ...
            # exclude invalid samples (start node has no edge?)
            mask = tails != -1   
...
class NeighborSampler(object):
    def __init__(
        ...
        # generate a homogeneous graph of item nodes
        # where the neighbors of each given node are the most commonly visited item nodes by multiple random walks starting from that given node
        # separate samplers -> different neighbors for each layer
        self.samplers = [
    ...
    def sample_blocks(self, seeds, heads=None, tails=None, neg_tails=None):
        ...
        for sampler in self.samplers:
            # the neighbor-0graph constructed by PinSAGESampler
            frontier = sampler(seeds)
            if heads is not None:
                # the neighbors of a query item should not include the sampled positive item or the negative item
                # which are used in the loss function
            ...
            # Convert the neighbor-graph into a bipartite-structured block for message passing
            block = compact_and_copy(frontier, seeds)
            # ids of all related nodes in the neighbor-graph
    ...
    def sample_from_item_pairs(self, heads, tails, neg_tails):
        ...
        # compact_graphs: Given a list of graphs with the same set of nodes, find and eliminate the common isolated nodes across all graphs
        pos_graph, neg_graph = dgl.compact_graphs([pos_graph, neg_graph])
        # ids of all nodes in pos_graph
        # need to generate neighbors of these nodes
